...
\emph{blurring} to remove the noise. We \emph{threshold} it again to get
the binary mask (see the results in \ref{fig:simple-background-second-threshold}).
At this point, we will find a contour with the biggest area using OpenCV
\todo[inline]{M: footnote se jmény funkcí? }
library. The center point of the bounding box of this contour will be our
estimation of the position of our object in the image. The whole process is illustrated in
...
\end{figure}
\todo[inline]{M: proč uvozovky?, možná místo by něco jako source}

We describe the algorithm in a few steps. First, we convert the template
image (bounding box) from the RGB color space to HSV. Then we calculate the average
...
To get a more reasonable average, we take the hue value of each pixel as a
unit vector (the hue value is encoded as the angle between given vector
and vector $(1, 0)$). We sum these vectors and get a vector $(x, y)$. We find the
corresponding angle for this vector using $arctang2$ function. The following
\todo[inline]{M: nejmenuje se to atan2?}
...
\todo[inline]{M: chybí tady popis, jak se pak s tou průměrnou hodnotou hledá ten
objekt (přesná shoda hue, nějaká tolerance?}
...
\todo[inline]{M: jak je definovaná vzdálenost pixelů?}
...
such conversion).
\todo[inline]{M: tenhle odstavec bych dal před ten vzorec, viz má předchozí poznímka}
\todo[inline]{M: co je "standard conversion to grayscale"?}
...
that also in the case of multiple moving objects in a frame it keeps tracking the original
one.
...
The MIL is an abbreviation for Multiple Instance Learning. In comparison with the
...
\todo[inline]{M: "we decided" -- proč? zdůvodnění}
...
% pozor na precision vs accuracy, je to rozdíl
...
This value is highly dependent on the hardware and the operating system.
Therefore, we use the value of FPS as a rough guess.
\todo[inline]{M: místo operating system bych dal overall load of the system}
...
The goal of the first experiment is to estimate tracker speed and accuracy. We used a
...
the HSV tracker we choosed to track the blue battery which has one-color area.
\todo[inline]{M: na obrázku je oranžový vršek a ne modrá baterka}
...
\end{table}
\todo[inline]{M: možná bych tu inaccurracy oznařil písmenkem a pak dal $\bar{X}, \sigma_X$ do tabulky}
\todo[inline]{M: uvést v tabulce ty jednotky (v textu jsou příliš bokem)}
\todo[inline]{M: sloupce FPS na Speed (FPS)}
\todo[inline]{M: čísla zarovnat vpravo (nebo lépe na desetinnou tečku)}
\todo[inline]{M: možná seřadit tabulku podle zvoleného sloupce}
\todo[inline]{M: vytučnit v každém sloupci nejlepší hodnotu (mimo representative trackeru)}
...
\todo[inline]{M: HSV má 38.5, v tabulce jsou i horší, proč je to "not perform at all"?}

We can also see that, althouth the MIL and BOOSTING trackers were slow, they
performed quite well regarding accuracy. The trackers which have mean value of inaccuracy more than
100 px lost the object at some point and were not aware of it.
...
\todo[inline]{M: nadefinuj si pro mě \TeX makra a ta všude použij, změnit to
pak lze. Asi bych je při první zmínce pojmenovat a přitom zavedlě nějakou
zkraktu pro další použití.}
