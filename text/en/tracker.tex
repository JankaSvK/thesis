...
cropped image to the size of the template.
...
\todo[inline]{Square distance, ten vzorec hovori co?}
\todo[inline]{Obrazok kde bude patch, pattern, image}
...
precisely does $T(x, y)$ stand for). In general we can choose any reasonable
vector (such as tuple of RGB channels). We choose to use a standard
conversion to grayscale ($T(x, y)$ thus gives an intensity after such
conversion).
