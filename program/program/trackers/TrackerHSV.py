...
class TrackerHSV(object):
    ...
    def init(self, image, bbox):
        pattern = image[bbox[1]:bbox[3] + bbox[1],
                  bbox[0]:bbox[2] + bbox[0]]  # swapping x and y, since matrix representation have columns first
        ...
        self.lower_bound = numpy.array(
            [self.most_common_color - self.color_tolerancy, self.bottom_saturation, self.bottom_value])
        self.upper_bound = numpy.array(
            [self.most_common_color + self.color_tolerancy, self.upper_saturation, self.upper_value])
        return True
    ...
    def update(self, image):
        ...
        if self.lower_bound[0] < 0 or self.upper_bound[0] >= 180:
            if self.lower_bound[0] < 0:
                mask_add = cv2.inRange(hsv_image, [180, 0, 0] + self.lower_bound,
                                       numpy.array(self.highest_point).astype('float'))
            else:
